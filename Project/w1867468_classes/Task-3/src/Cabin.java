public class Cabin {
    ...
    public Cabin(WaitingQueue waitingList) {
        ...
        for ( int x = 1; x < (cabins.length + 1); x = x + 1 ) {
            for ( int m = 0; m < 3; m = m + 1 ) {
                if ( (cabins[x - 1].passengers[m].getFirstName().equalsIgnoreCase(firstName)) && (cabins[x - 1].passengers[m].getSureName().equalsIgnoreCase(sureName)) ) {
                    ...
                    // Adding passenger to the cabin from the waiting list:
                    if ( (cabins[x - 1].passengers[0].getFirstName().equalsIgnoreCase("-")) && (cabins[x - 1].passengers[1].getFirstName().equalsIgnoreCase("-")) && (cabins[x - 1].passengers[2].getFirstName().equalsIgnoreCase("-")) ) {
                        String toShip = waitingList.deQueue();
                        if ( toShip != null ) {
                            String deQueueParts[] = toShip.split(" ");
                            String fName = deQueueParts[0];
                            String sName = deQueueParts[1];
                            double pExpense = Double.parseDouble(deQueueParts[2]);

                            cabins[x - 1].passengers[m].setFirstName(fName);
                            cabins[x - 1].passengers[m].setSureName(sName);
                            cabins[x - 1].passengers[m].setExpenses(pExpense);
                        }
                    }
